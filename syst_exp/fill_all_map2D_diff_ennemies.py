...
#
    map_2d = create_maps_with_different_enemies(map2D_origin, mode_dev=True)
    if (map_2d is None) :
    #
        if mode_dev :
    ...
    for map in map_2d:
    #
        result = calculate_player_to_enemy_moves(map, mode_dev=True)
        if result is False  :
        #
            if mode_dev :
                print_red("The map is None.")
    ...
    if (create_json_file(data, 'data/map2D/map2D_diff_ennemies.json', mode_dev=True) == None) :
    #
